...
const DEFAULT_CONFIG = {
  ...
  edge: {
    ...
    glow: 22,
    opacity: 0.4,
    glowDirection: "out", // in | out — page-edge glow direction (default outward)
    flow: false,
    flowSpeed: 6,
  },
  badge: {
...
async function loadStore() {
  const stored = await loadJson(CONFIG_PATH, {});
  config = { ...structuredClone(DEFAULT_CONFIG), ...stored };
  config.edge = { ...structuredClone(DEFAULT_CONFIG.edge), ...(config.edge ?? {}) };
  // legacy migration: edge.composer { enabled, glowDirection } → top-level composer
  if (stored && stored.composer === undefined && stored.edge && stored.edge.composer) {
    config.composer.enabled = stored.edge.composer.enabled !== false;
    config.composer.glowDirection = ["in", "out"].includes(stored.edge.composer.glowDirection)
      ? stored.edge.composer.glowDirection
      : DEFAULT_CONFIG.composer.glowDirection;
  }
  config.composer = { ...structuredClone(DEFAULT_CONFIG.composer), ...(config.composer ?? {}) };
  // legacy migration: edge.badge (boolean) → top-level badge
  if (stored && stored.badge === undefined && stored.edge && typeof stored.edge.badge === "boolean") {
    config.badge.enabled = stored.edge.badge;
  }
  config.badge = {
    enabled: config.badge.enabled !== false,
    desktop: { ...DEFAULT_CONFIG.badge.desktop, ...(config.badge?.desktop ?? {}) },
    mobile: { ...DEFAULT_CONFIG.badge.mobile, ...(config.badge?.mobile ?? {}) },
  };
...
function sanitizeConfig(input) {
  ...
  out.edge.glowDirection = ["in", "out"].includes(edge.glowDirection) ? edge.glowDirection : "out";

  // status badge: fully independent block (PC / mobile corner + offsets)
  const bd = input.badge ?? {};
  out.badge.enabled = bd.enabled !== false;
  const cornerOf = (v) => (["tl", "tr", "bl", "br"].includes(v) ? v : "br");
  out.badge.desktop = {
    corner: cornerOf(bd.desktop?.corner),
    x: clampInt(bd.desktop?.x, 0, 400, DEFAULT_CONFIG.badge.desktop.x),
    y: clampInt(bd.desktop?.y, 0, 400, DEFAULT_CONFIG.badge.desktop.y),
  };
  out.badge.mobile = {
    corner: cornerOf(bd.mobile?.corner),
    x: clampInt(bd.mobile?.x, 0, 400, DEFAULT_CONFIG.badge.mobile.x),
    y: clampInt(bd.mobile?.y, 0, 400, DEFAULT_CONFIG.badge.mobile.y),
  };

  // input-box glow: fully independent appearance block
  const comp = input.composer ?? {};
  out.composer.enabled = comp.enabled !== false;
  out.composer.peakColor = /^#[0-9a-fA-F]{6}$/.test(String(comp.peakColor ?? "")) ? comp.peakColor : DEFAULT_CONFIG.composer.peakColor;
  out.composer.valleyColor = /^#[0-9a-fA-F]{6}$/.test(String(comp.valleyColor ?? "")) ? comp.valleyColor : DEFAULT_CONFIG.composer.valleyColor;
  out.composer.width = clampInt(comp.width, 1, 14, DEFAULT_CONFIG.composer.width);
  out.composer.animation = ["breathing", "solid", "off"].includes(comp.animation) ? comp.animation : DEFAULT_CONFIG.composer.animation;
  out.composer.breathingSpeed = clampNum(comp.breathingSpeed, 0.5, 10, DEFAULT_CONFIG.composer.breathingSpeed);
  out.composer.glow = clampInt(comp.glow, 0, 80, DEFAULT_CONFIG.composer.glow);
  out.composer.opacity = clampNum(comp.opacity, 0.2, 1, DEFAULT_CONFIG.composer.opacity);
  out.composer.glowDirection = ["in", "out"].includes(comp.glowDirection) ? comp.glowDirection : DEFAULT_CONFIG.composer.glowDirection;
  out.composer.flow = comp.flow === true;
  out.composer.flowSpeed = clampNum(comp.flowSpeed, 2, 24, DEFAULT_CONFIG.composer.flowSpeed);
  ...
  return {
    ...
    status: async () => {
      ...
      return {
        ...
        badge: {
          enabled: config.badge.enabled !== false,
          desktop: { ...config.badge.desktop },
          mobile: { ...config.badge.mobile },
        },
    ...
    reset: async (payload) => {
      ...
      if (section === "" || section === "all") {
        ...
        config.composer = structuredClone(DEFAULT_CONFIG.composer);
      } else if (section === "badge") {
        config.badge = structuredClone(DEFAULT_CONFIG.badge);

/**
 * chicheng-peak-valley — host half
 *
...
const name = "chicheng-peak-valley";
...
async function flushConfig() {
  try {
    ...
    console.error("[chicheng-peak-valley] config flush failed:", error);
  }
}
...
async function flushFired() {
  try {
    ...
    console.error("[chicheng-peak-valley] fired flush failed:", error);
  }
}
...
async function listPushChannels(ctx) {
  try {
    ...
    console.warn("[chicheng-peak-valley] push channels (service) failed:", error instanceof Error ? error.message : String(error));
  }
  try {
    const port = ctx?.webServer?.port ?? 3080;
    ...
    console.warn("[chicheng-peak-valley] push channels (http) failed:", error instanceof Error ? error.message : String(error));
  }
  return [];
}
...
async function listMessagingTargets(ctx) {
  try {
    ...
    console.warn("[chicheng-peak-valley] messaging targets failed:", error instanceof Error ? error.message : String(error));
  }
  try {
    const messaging = resolveMessaging(ctx);
    ...
    console.warn("[chicheng-peak-valley] messaging status (service) failed:", error instanceof Error ? error.message : String(error));
  }
...
async function fireReminder(ctx, phase) {
  ...
  for (const s of starts) {
    ...
    r.lastResult = result;
    scheduleSave();
    console.info(`[chicheng-peak-valley] reminder fired (${key}) ok=${result?.ok === true}`);
  }
  ...
  if (times.length > 0) {
    ...
    if (currentPhase === phase) {
      ...
      for (const min of times) {
        ...
        r.lastResult = result;
        scheduleSave();
        console.info(`[chicheng-peak-valley] daily reminder fired (${key}) ok=${result?.ok === true}`);
      }
    }
  }
}
...
async function onTick() {
  ...
  try {
    ...
    console.warn("[chicheng-peak-valley] tick failed:", error instanceof Error ? error.message : String(error));
  }
}
...
async function apply(ctx, configArg) {
  ...
  }), "chicheng-peak-valley: /peakvalley/api routes");
  ...
  ctx.effect(() => () => {
    ...
    void flushConfig();
    void flushFired();
  }, "chicheng-peak-valley: teardown");

  ctx.logger?.info?.("[chicheng-peak-valley] started, data root: " + DATA_ROOT);
}

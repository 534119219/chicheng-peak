...
const DEFAULT_CONFIG = {
  ...
  edge: {
    ...
    glow: 22,
    opacity: 0.4,
    glowDirection: "in", // in | out — page-edge glow direction
    flow: false,
    flowSpeed: 6,
    badge: true,
    composer: {
      enabled: true,
      glowDirection: "in", // in | out — input-box glow direction
    },
  },
  localNotify: {
    enabled: false,
...
function sanitizeConfig(input) {
  ...
  out.edge.glowDirection = ["in", "out"].includes(edge.glowDirection) ? edge.glowDirection : "in";
  const comp = edge.composer ?? {};
  out.edge.composer.enabled = comp.enabled !== false;
  out.edge.composer.glowDirection = ["in", "out"].includes(comp.glowDirection) ? comp.glowDirection : "in";

...
const DEFAULT_CONFIG = {
  ...
  edge: {
    ...
    composer: {
      enabled: true,
      glowDirection: "out", // in | out — input-box glow direction (default outward)
    },
  },
  localNotify: {
    enabled: false,
...
function sanitizeConfig(input) {
  ...
  const comp = edge.composer ?? {};
  out.edge.composer.enabled = comp.enabled !== false;
  out.edge.composer.glowDirection = ["in", "out"].includes(comp.glowDirection) ? comp.glowDirection : "out";

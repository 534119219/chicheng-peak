window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Composer border: the chat input wears the same phase-colored breathing
     * border as the page edge, with its own glow direction. Re-positioned on
     * every poll and on scroll/resize/focus events so it hugs the input even
     * while the mobile keyboard animates.
     */
    function updateComposerRing(s, edge, color, glow, opacity, speed, w) {
      try {
        ...
        var cring = els.cring;
        var on = s && s.enabled && edge && edge.enabled !== false && edge.composer && edge.composer.enabled !== false;
        if (!on) {
        ...
        var radius = "12px";
        try {
          var cs = window.getComputedStyle ? window.getComputedStyle(el).borderRadius : "";

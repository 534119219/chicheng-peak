window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     *  that holds the input plus its toolbar/send row. Returns null when no
     *  wrapping container is clearly identifiable; callers fall back to the
     *  input itself. */
    function findComposerBox(input) {
      ...
      for (var hops = 0; node && hops < 10; hops += 1) {
        ...
        var bigger = r.height >= tR.height + 16;
        if (bigger) {
          var name = "";
          try {
          ...
          if (/composer|input|chat|ask|send|toolbar|bar|box/i.test(name)) return node;
        }

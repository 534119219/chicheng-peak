window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var runtime = {
      ...
      poll: function () {
        api("status").then(function (s) {
          ...
          runtime.publish();
          try { console.info("[peak-valley] status:", s.phase, s.edge && s.edge.enabled ? "edge-on" : "edge-off"); } catch (e) { /* ignore */ }
        }).catch(function () {
          hideOverlay();
          try { console.warn("[peak-valley] status fetch failed"); } catch (e) { /* ignore */ }
        });
      },
    ...
    function sendLocalNotification(title, body) {
      ...
      try {
        new Notification(String(title), { body: String(body), tag: "chicheng-peak-valley", silent: false });
      } catch (e) { /* ignore */ }
    }
    ...
    function overlayElements() {
      ...
      root.style.cssText = "position:fixed;inset:0;pointer-events:none;z-index:2147483000;opacity:0;transition:opacity .3s ease;visibility:hidden;";
      var ring = document.createElement("div");
      ring.id = "dsh-pv-ring";
      ring.style.cssText = "position:absolute;inset:8px;border-radius:20px;box-sizing:border-box;";
      var comet1 = document.createElement("div");
      comet1.id = "dsh-pv-comet-1";
      comet1.style.cssText = "position:absolute;width:26px;height:26px;border-radius:999px;pointer-events:none;offset-path:inset(8px round 20px);offset-rotate:0deg;";
      var comet2 = document.createElement("div");
      comet2.id = "dsh-pv-comet-2";
      comet2.style.cssText = "position:absolute;width:20px;height:20px;border-radius:999px;pointer-events:none;offset-path:inset(8px round 20px);offset-rotate:0deg;";
      var badge = document.createElement("div");
      badge.id = "dsh-pv-badge";
      badge.style.cssText = "position:fixed;right:18px;bottom:18px;display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:5px 12px;font-size:12px;font-weight:600;line-height:1.5;box-shadow:0 2px 12px rgba(0,0,0,.25);backdrop-filter:blur(6px);z-index:2147483000;";
      root.appendChild(ring);
      root.appendChild(comet1);
      root.appendChild(comet2);
      document.documentElement.appendChild(root);
      document.documentElement.appendChild(badge);
      overlayEls = { root: root, ring: ring, comet1: comet1, comet2: comet2, badge: badge, styleEl: null };
      return overlayEls;
    }
    ...
    function ensureStyleEl() {
      ...
      style.id = STYLE_ID;
      style.textContent = [
        "@keyframes dsh-pv-breathe{0%,100%{box-shadow:0 0 var(--dsh-pv-glow-lo,10px) var(--dsh-pv-color,#f97316);opacity:var(--dsh-pv-op-lo,.5)}50%{box-shadow:0 0 var(--dsh-pv-glow,22px) var(--dsh-pv-color,#f97316);opacity:var(--dsh-pv-op-hi,.95)}}",
    ...
    function applyOverlay(s, isPreview) {
      ...
      try {
        ...
        if (edge.animation === "breathing") {
          ring.style.animation = "dsh-pv-breathe " + speed + "s ease-in-out infinite";
        } else {
          ring.style.animation = "none";
          ring.style.opacity = String(opacity);
          ring.style.boxShadow = edge.animation === "solid" ? "0 0 " + glow + "px " + color : "none";
        }
    ...
    function PeakValleySection(props) {
      ...
      var edgePreview = createElement("div", { style: { display: "flex", flexDirection: "column", gap: "6px", flex: "none", width: 120 } },
        ...
        createElement("div", {
          style: {
            width: 120, height: 64, borderRadius: 12, boxSizing: "border-box",
            border: edge.width + "px solid " + (phase === "peak" ? edge.peakColor : edge.valleyColor),
            boxShadow: "0 0 " + (edge.glow || 0) + "px " + (phase === "peak" ? edge.peakColor : edge.valleyColor),
    ...
    function apply(ctx) {
      ...
      }, "chicheng-peak-valley: locale");
      var t = ctx.locale.bind(NS);

      ctx.effect(function () {
        runtime.start();
        try { console.info("[peak-valley] overlay engine started"); } catch (e) { /* ignore */ }
        return function () {
          runtime.stop();
          try { console.info("[peak-valley] overlay engine stopped"); } catch (e) { /* ignore */ }
        };
      }, "chicheng-peak-valley: overlay engine");

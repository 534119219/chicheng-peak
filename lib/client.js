window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function overlayElements() {
      ...
      cring.style.cssText = "position:fixed;pointer-events:none;box-sizing:border-box;visibility:hidden;z-index:2147483000;";
     ...
     *  that holds the input plus its toolbar/send row. Selection rules (bottom
     *  up; candidates must fully contain the textarea):
     *   1. keyword hit (word-boundary composer/input/ask/command/send/toolbar
     *      — "sidebar" can never match) with a moderate height ratio (1.15–4);
     *   2. any moderate-ratio ancestor (1.3–4) — the visible composite box;
     *   3. the smallest wrapper that is clearly taller than the textarea;
     *   → otherwise null, and callers fall back to the input itself. */
    function findComposerBox(input) {
      ...
      for (var hops = 0; node && hops < 10; hops += 1) {
        ...
        candidates.push({ node: node, ratio: r.height / tR.height, kw: kw });
        node = node.parentElement;
      }
      for (var i = 0; i < candidates.length; i += 1) {
        var c = candidates[i];
        if (c.kw && c.ratio >= 1.15 && c.ratio <= 4) return c.node;
      }
      for (var j = 0; j < candidates.length; j += 1) {
        var c2 = candidates[j];
        if (c2.ratio >= 1.3 && c2.ratio <= 4) return c2.node;
      }
      if (candidates.length > 0 && candidates[0].ratio >= 1.15) return candidates[0].node;
      return null;
    }
    ...
    function updateComposerRing(s, edge, color, glow, opacity, speed, w) {
      try {
        ...
        var compDir = edge.composer.glowDirection === "out" ? "out" : "in";

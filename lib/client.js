    ...
    function updateComposerRing(s) {
      try {
        ...
        if (!on) {
          ...
          try { console.info("[chicheng-peak] composer ring: hidden (disabled: master=" + (s && s.enabled) + " composer=" + (c && c.enabled) + ")"); } catch (e) { /* ignore */ }
          return;
        }
        var el = findComposer(false);
        if (!el) {
          // Transient state (popup/modal covering or hiding the composer):
          // keep the last known geometry so the glow does not flicker away.
          // It sits at z 30, below overlay masks, so a stale rect is harmless.
          if (!runtime._composerLast) {
            cring.style.visibility = "hidden";
            try { console.info("[chicheng-peak] composer ring: hidden (no input found, no cache)"); } catch (e) { /* ignore */ }
          }
          return;
        }
        ...
        if (!r || r.width < 40 || r.height < 16 || r.height <= 0) {
          if (!runtime._composerLast) {
            cring.style.visibility = "hidden";
            try { console.info("[chicheng-peak] composer ring: hidden (invalid rect " + (r && Math.round(r.width) + "x" + Math.round(r.height)) + ")"); } catch (e) { /* ignore */ }
            return;
          }
        ...
        try { console.info("[chicheng-peak] composer ring: render " + Math.round(r.left) + "," + Math.round(r.top) + " " + Math.round(r.width) + "x" + Math.round(r.height) + " radius=" + radius + " input=" + (el.tagName || el.nodeName)); } catch (e) { /* ignore */ }

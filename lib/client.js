window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var runtime = {
      ...
      start: function () {
        ...
        var onResize = function () {
          if (overlayEls) placeBadgeAboveComposer(overlayEls.badge);
        };
    ...
    function ensureStyleEl() {
      ...
      style.id = STYLE_ID;
      style.textContent = [
        "@keyframes dsh-pv-breathe{0%,100%{box-shadow:inset 0 0 var(--dsh-pv-glow-lo,10px) var(--dsh-pv-color,#f97316)}50%{box-shadow:inset 0 0 var(--dsh-pv-glow,22px) var(--dsh-pv-color,#f97316)}}",
        "@keyframes dsh-pv-orbit{from{offset-distance:0%}to{offset-distance:100%}}",
        "#dsh-pv-comet-1,#dsh-pv-comet-2{visibility:hidden}",
        "#dsh-pv-badge{position:fixed;right:18px;bottom:18px;display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:5px 12px;font-size:12px;font-weight:600;line-height:1.5;box-shadow:0 2px 12px rgba(0,0,0,.25);backdrop-filter:blur(6px);z-index:2147483000;visibility:hidden}",
    ...
    /** Find the chat composer input (lowest visible text input on screen). */
    function findComposer() {
      try {
        var nodes = document.querySelectorAll('main textarea, main [contenteditable="true"], [role="textbox"], textarea');
        var best = null;
        for (var i = 0; i < nodes.length; i += 1) {
          var n = nodes[i];
          var r = n.getBoundingClientRect();
          if (!r || r.width < 40 || r.height < 16) continue;
          if (r.bottom > window.innerHeight - 4 || r.top < 0) continue;
          if (best === null || r.bottom > best.bottom) best = { el: n, bottom: r.bottom, top: r.top };
        }
    ...
    /**
     * Mobile: pin the badge just above the composer input. Desktop keeps the
     * bottom-right corner (stylesheet default). Falls back to the media-query
     * position when no input can be detected.
     */
    function placeBadgeAboveComposer(badge) {
      if (!badge || !isMobileish()) return;
      try {
        var el = findComposer();
        if (!el) { badge.style.bottom = ""; return; }
        var r = el.getBoundingClientRect();
        if (!r || r.height <= 0) { badge.style.bottom = ""; return; }
        badge.style.bottom = Math.max(10, Math.round(window.innerHeight - r.top + 6)) + "px";
      } catch (e) { /* ignore */ }
    }
    ...
    function applyOverlay(s, isPreview) {
      ...
      try {
        ...
        if (edge.animation === "breathing") {
          // breathing glow breathes on the INNER edge of the border
          ring.style.animation = "dsh-pv-breathe " + speed + "s ease-in-out infinite";
        } else {
          ring.style.animation = "none";
          ring.style.boxShadow = edge.animation === "solid" ? "inset 0 0 " + glow + "px " + color : "none";
        }

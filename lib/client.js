window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      glowOut: "向外",
      composerTitle: "输入框光效",
      composerDesc: "输入框控件的独立光效分区：颜色、宽度、光晕、呼吸与流光均可与页面边缘不同。",
      composerEnabled: "启用输入框光效",
      composerGlowDir: "光效方向（输入框）",
      resetComposer: "恢复默认光效",
      flow: "流光效果",
      flowSpeed: "流光速度（秒/圈）",
      badgeTitle: "状态角标",
      badgeDesc: "独立的阶段状态角标（高峰/低峰），不随页面边框开关联动；PC 与手机端可分别选择四角位置并调整偏移。",
      badgeEnabled: "启用状态角标",
      badgeDesktop: "PC 端",
      badgeMobile: "手机端",
      badgeCorner: "所在角落",
    ...
    var en = {
      ...
      glowIn: "Inward",
      glowOut: "Outward",
      composerTitle: "Composer Glow",
      composerDesc: "Independent glow for the input control: colors, width, glow, breathing and flow are all separate from the page edge.",
      composerEnabled: "Enable composer glow",
      composerGlowDir: "Glow direction (composer)",
      resetComposer: "Reset glow",
      flow: "Flowing light",
      flowSpeed: "Flow speed (s/rev)",
      badgeTitle: "Status Badge",
      badgeDesc: "Independent phase badge (peak/valley) — not tied to the page-edge toggle. Pick a corner and offsets for PC and mobile separately.",
      badgeEnabled: "Enable status badge",
      ...
      badgeX: "Horizontal offset",
      badgeY: "Vertical offset",
      resetBadge: "Reset position",
      preview: "Preview",
    ...
    var runtime = {
      ...
      start: function () {
        ...
        // Lightweight local re-measure (no network): bounds the composer
        // glow/badge lag to ~2s for layout moves that fire no events (e.g.
        // toggling the sidebars reflows the composer without resize/scroll).
        runtime._repositionTimer = setInterval(function () {
          repositionDynamic();
        }, 2000);
        if (runtime._repositionTimer && runtime._repositionTimer.unref) runtime._repositionTimer.unref();
        var onVis = function () {
        ...
        // Dialog open/close mutates the DOM — watch it so the composer glow
        // restores INSTANTLY when a modal/picker closes (no 2s wait).
        runtime._dialogOpen = appDialogOpen();
        try {
          if (typeof MutationObserver !== "undefined") {
            runtime._dialogMO = new MutationObserver(function () {
              var open = appDialogOpen();
              if (open !== runtime._dialogOpen) {
                runtime._dialogOpen = open;
                repositionDynamic();
              }
            });
            runtime._dialogMO.observe(document.documentElement, { childList: true, subtree: true });
          }
        } catch (e) { /* ignore */ }
      },
      stop: function () {
        if (runtime.timer !== null) { clearTimeout(runtime.timer); runtime.timer = null; }
        if (runtime._repositionTimer !== null) { clearInterval(runtime._repositionTimer); runtime._repositionTimer = null; }
        ...
        if (runtime._dialogMO) {
          try { runtime._dialogMO.disconnect(); } catch (e2) { /* ignore */ }
          runtime._dialogMO = null;
          runtime._dialogOpen = false;
        }
    ...
    function ensureStyleEl() {
      ...
      style.id = STYLE_ID;
      style.textContent = [
        "@keyframes dsh-pv-breathe-in{0%,100%{box-shadow:inset 0 0 var(--dsh-pv-glow-lo,10px) var(--dsh-pv-glow-color,var(--dsh-pv-color,#f97316))}50%{box-shadow:inset 0 0 var(--dsh-pv-glow,22px) var(--dsh-pv-glow-color,var(--dsh-pv-color,#f97316))}}",
        "@keyframes dsh-pv-breathe-out{0%,100%{box-shadow:0 0 var(--dsh-pv-glow-lo,10px) var(--dsh-pv-glow-color,var(--dsh-pv-color,#f97316))}50%{box-shadow:0 0 var(--dsh-pv-glow,22px) var(--dsh-pv-glow-color,var(--dsh-pv-color,#f97316))}}",
        "@keyframes dsh-pv-orbit{from{offset-distance:0%}to{offset-distance:100%}}",
        "#dsh-pv-comet-1,#dsh-pv-comet-2{visibility:hidden}",
        "#dsh-pv-badge{position:fixed;display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:5px 12px;font-size:12px;font-weight:600;line-height:1.5;box-shadow:0 2px 12px rgba(0,0,0,.25);backdrop-filter:blur(6px);z-index:30;visibility:hidden}",
        // composer glow lives on the container itself via data attrs + root
        // CSS vars — React re-renders never touch either, so the glow follows
        // the input in real time with zero tracking lag
        "[data-dsh-pv]{border:var(--dsh-pv-bw,2px) solid var(--dsh-pv-glow-color,var(--dsh-pv-color,#f97316))!important;box-shadow:var(--dsh-pv-shadow,none)!important}",
        "[data-dsh-pv-anim=\"breathe-in\"]{animation:dsh-pv-breathe-in var(--dsh-pv-speed,2.6s) ease-in-out infinite!important}",
        "[data-dsh-pv-anim=\"breathe-out\"]{animation:dsh-pv-breathe-out var(--dsh-pv-speed,2.6s) ease-in-out infinite!important}",
        "[data-dsh-pv-anim=\"none\"]{animation:none!important}",
      ].join("\n");
    ...
    /** True when a sizable app dialog (settings modal, pickers, popups) is
     *  open — the composer is covered then, and its own inputs must never be
     *  mistaken for the composer. */
    function appDialogOpen() {
      try {
        var dialogs = document.querySelectorAll('[aria-modal="true"], [role="dialog"]');
        for (var i = 0; i < dialogs.length; i += 1) {
          var r = dialogs[i].getBoundingClientRect();
          if (r && r.width > 100 && r.height > 100) return true;
        }
      } catch (e) { /* ignore */ }
      return false;
    }

    /** True when the element sits inside an open dialog container. */
    function insideDialog(el) {
      try {
        if (el && el.closest) return !!el.closest('[aria-modal="true"], [role="dialog"]');
      } catch (e) { /* ignore */ }
      return false;
    }
     ...
     *  so it picks the lowest visible input-like element. Inputs inside open
     *  app dialogs are never candidates. */
    function findComposer(preferActive) {
      try {
        ...
        if (preferActive !== false && active && (active.tagName === "TEXTAREA" || active.tagName === "INPUT" || active.isContentEditable)) {
          ...
          if (ar && ar.width >= 40 && ar.height >= 16 && ar.top >= 0 && ar.bottom <= vh && !insideDialog(active)) {
            return active;
          }
        }
    ...
    /** Position the status badge at a configurable corner (tl/tr/bl/br) with
     *  x/y offsets, using the PC or mobile profile. Runs on every poll and on
     *  resize/viewport events. */
    function placeBadge(badge, cfg) {
      if (!badge) return;
      try {
        var p = (isMobileish() ? (cfg && cfg.mobile) : (cfg && cfg.desktop)) || { corner: "br", x: 18, y: 18 };
        var corner = p.corner || "br";
        var x = Math.max(0, Math.round(Number(p.x) || 0));
        var y = Math.max(0, Math.round(Number(p.y) || 0));
        badge.style.left = (corner === "tl" || corner === "bl") ? x + "px" : "";
        badge.style.right = (corner === "tr" || corner === "br") ? x + "px" : "";
        badge.style.top = (corner === "tl" || corner === "tr") ? y + "px" : "";
        badge.style.bottom = (corner === "bl" || corner === "br") ? y + "px" : "";
      } catch (e) { /* ignore */ }
    }

    /**
     * Composer glow: the composer CONTROL (input box incl. its toolbar/send
     * row) wears its OWN phase-colored glow — colors, width, glow, breathing,
     * flow and direction are independent from the page edge (top-level
     * `composer` config). Re-positioned on every poll and on input/resize/
     * scroll/focus events so it hugs the control even while the textarea
     * auto-grows or the mobile keyboard animates.
     */
    /** Convert #rrggbb to rgba() with the given alpha (for glow opacity). */
    ...
    /** Remove the composer glow styles (data attrs + root CSS vars). */
    function clearComposerStyles() {
      try {
        if (runtime._composerStyledBox) {
          runtime._composerStyledBox.removeAttribute("data-dsh-pv");
          runtime._composerStyledBox.removeAttribute("data-dsh-pv-anim");
          runtime._composerStyledBox = null;
        }
        var r = document.documentElement;
        r.style.removeProperty("--dsh-pv-color");
        r.style.removeProperty("--dsh-pv-glow-color");
        r.style.removeProperty("--dsh-pv-glow");
        r.style.removeProperty("--dsh-pv-glow-lo");
        r.style.removeProperty("--dsh-pv-speed");
        r.style.removeProperty("--dsh-pv-bw");
        r.style.removeProperty("--dsh-pv-shadow");
      } catch (e) { /* ignore */ }
    }

    /**
     * Composer glow: the composer CONTROL (input box incl. its toolbar/send
     * row) wears its OWN phase-colored glow. The border/glow/animation are
     * applied DIRECTLY to the container via a stylesheet rule
     * ([data-dsh-pv]) + CSS variables on <html> — React re-renders never
     * touch either, so the glow lives on the element itself and follows the
     * layout in real time with zero tracking lag. Flow comets stay on the
     * overlay (decorative, optional).
     */
    function updateComposerRing(s) {
      try {
        var els = overlayEls;
        if (!els || !els.cring) return;
        var cring = els.cring;
        var c = (s && s.composer) || null;
        // defensive: tolerate hosts that still expose the legacy edge.composer
        if (!c && s && s.edge && s.edge.composer) {
          c = {
            enabled: s.edge.composer.enabled !== false,
            glowDirection: s.edge.composer.glowDirection === "in" ? "in" : "out",
          };
        }
        var on = !!(s && s.enabled && (!c || c.enabled !== false));
        if (!on) {
          clearComposerStyles();
          cring.style.visibility = "hidden";
          runtime._composerLast = null;
          try { console.info("[chicheng-peak] composer glow: off (master=" + (s && s.enabled) + " composer=" + (c && c.enabled) + ")"); } catch (e) { /* ignore */ }
          return;
        }
        // An open app dialog (settings modal, pickers) covers the composer —
        // and its own inputs would otherwise match the composer scan when
        // the panel scrolls. Skip the glow entirely while a dialog is open.
        if (appDialogOpen()) {
          clearComposerStyles();
          cring.style.visibility = "hidden";
          runtime._composerLast = null;
        ...
        // findComposer must never target inputs inside an open dialog either
        var el = findComposer(false);
        if (!el) {
          // Transient state (popup/modal covering or hiding the composer):
          // keep the last known geometry for the comet overlay.
          if (!runtime._composerLast) {
            clearComposerStyles();
            cring.style.visibility = "hidden";
            try { console.info("[chicheng-peak] composer glow: hidden (no input found, no cache)"); } catch (e) { /* ignore */ }
          }
          return;
        }
        var box = findComposerBox(el) || el;
        keepComposerObserved(box);
        ...
        if (!r || r.width < 40 || r.height < 16 || r.height <= 0) {
          if (!runtime._composerLast) {
            cring.style.visibility = "hidden";
            try { console.info("[chicheng-peak] composer glow: hidden (invalid rect " + (r && Math.round(r.width) + "x" + Math.round(r.height)) + ")"); } catch (e) { /* ignore */ }
            return;
          }
          r = runtime._composerLast;
          radius = r.radius;
          radiusPx = r.radiusPx;
        } else {
          runtime._composerLast = { left: r.left, top: r.top, width: r.width, height: r.height, radius: radius, radiusPx: radiusPx };
        }

        var color = s.phase === "peak" ? (c.peakColor || "#f97316") : (c.valleyColor || "#38bdf8");
        var glow = Math.max(0, Number(c.glow) || 0);
        var opacity = Math.min(1, Math.max(0.2, Number(c.opacity) || 0.4));
        var speed = Number(c.breathingSpeed) || 2.6;
        var w = Math.min(14, Math.max(1, Number(c.width) || 2));
        var compDir = c.glowDirection === "out" ? "out" : "in";

        // ---- direct styling on the composer container (moves with it)
        if (runtime._composerStyledBox && runtime._composerStyledBox !== box) {
          runtime._composerStyledBox.removeAttribute("data-dsh-pv");
          runtime._composerStyledBox.removeAttribute("data-dsh-pv-anim");
        }
        var rootEl = document.documentElement;
        rootEl.style.setProperty("--dsh-pv-color", color);
        rootEl.style.setProperty("--dsh-pv-glow-color", hexToRgba(color, opacity));
        rootEl.style.setProperty("--dsh-pv-glow", glow + "px");
        rootEl.style.setProperty("--dsh-pv-glow-lo", Math.max(2, Math.round(glow * 0.45)) + "px");
        rootEl.style.setProperty("--dsh-pv-speed", speed + "s");
        rootEl.style.setProperty("--dsh-pv-bw", w + "px");
        rootEl.style.setProperty("--dsh-pv-shadow", (compDir === "out" ? "0 0 " : "inset 0 0 ") + glow + "px " + hexToRgba(color, opacity));
        box.setAttribute("data-dsh-pv", "1");
        box.setAttribute("data-dsh-pv-anim", c.animation === "breathing" ? "breathe-" + compDir : "none");
        runtime._composerStyledBox = box;

        // ---- flow comets (overlay, decorative)
        var cc1 = els.ccomet1;
        var cc2 = els.ccomet2;
        if (c.flow === true) {
    ...
    /** Re-position everything that follows the composer (badge + glow ring)
     *  from the last known status. Called on scroll/resize/focus events. */
    function repositionDynamic() {
      var s = runtime.status;
      if (!s || !overlayEls) return;
      try {
        placeBadge(overlayEls.badge, s.badge);
        updateComposerRing(s);
      } catch (e) { /* ignore */ }
    }

    /** Hide only the page-edge layer (ring/comets); the composer glow and the
     *  status badge are independent and keep their own state. */
    function hidePageEdge() {
      var el = document.getElementById(OVERLAY_ID);
    ...
    function applyOverlay(s, isPreview) {
      ...
      try {
        ...
        var edge = s.edge;
        var edgeOn = !!(edge && edge.enabled !== false);
        if (edgeOn) {
        ...
        // page edge dims via ring element opacity, so the glow color stays full
        els.root.style.setProperty("--dsh-pv-glow-color", color);
        ...
        // status badge — independent layer (own enabled + PC/mobile corner)
        var badge = els.badge;
        var bd = s.badge || {};
        if (s.enabled && bd.enabled !== false) {
          ...
          badge.style.background = s.phase === "peak" ? (edge && edge.peakColor) || "#f97316" : (edge && edge.valleyColor) || "#38bdf8";
    ...
    function PeakValleySection(props) {
      ...
      var updateComposer = function (patch) {
        update({ composer: Object.assign({}, cp, patch) });
      };

      var updateBadge = function (patch) {
        update({ badge: Object.assign({}, bd, patch) });
      };

      var composerPreview = createElement("div", { style: { position: "relative", flex: "none", width: 120, height: 64 } },
        createElement("div", {
          style: {
            position: "absolute", inset: 0, borderRadius: 12, boxSizing: "border-box",
            border: (cp.width || 2) + "px solid " + (phase === "peak" ? cp.peakColor : cp.valleyColor),
            boxShadow: (cp.glowDirection === "out" ? "" : "inset ") + "0 0 " + (cp.glow || 0) + "px " + (phase === "peak" ? cp.peakColor : cp.valleyColor),
        ...
        createElement("span", {
          style: {
            ...
            fontSize: 12, color: "var(--dsw-alias-label-tertiary)", pointerEvents: "none",
          },
        }, t("preview"))
      );
      ...
      var edgePreview = createElement("div", { style: { position: "relative", flex: "none", width: 120, height: 64 } },
        createElement("div", {
          style: {
            position: "absolute", inset: 0, borderRadius: 8, boxSizing: "border-box",
      ...
      return createElement("div", { style: sectionStyle },
        ...
        createElement(Card, { title: t("edgeTitle"), desc: t("edgeDesc") },
          // 颜色 + 预览同一行
          createElement("div", { style: { display: "flex", alignItems: "flex-start", gap: "16px", flexWrap: "wrap" } },
            createElement("div", { style: { flex: "1 1 240px", minWidth: 0, display: "flex", flexDirection: "column", gap: "2px" } },
        ...
        // ---- 输入框光效（独立分区）
        createElement(Card, { title: t("composerTitle"), desc: t("composerDesc") },
          // 颜色 + 预览同一行
          createElement("div", { style: { display: "flex", alignItems: "flex-start", gap: "16px", flexWrap: "wrap" } },
            createElement("div", { style: { flex: "1 1 240px", minWidth: 0, display: "flex", flexDirection: "column", gap: "2px" } },
              createElement(ColorRow, {
                label: t("peakColor"),
                value: cp.peakColor || "#f97316",
                onChange: function (v) { updateComposer({ peakColor: v }); },
                swatches: ["#f97316", "#ea580c", "#f59e0b", "#ef4444", "#fb7185"],
              }),
              createElement(ColorRow, {
                label: t("valleyColor"),
                value: cp.valleyColor || "#38bdf8",
                onChange: function (v) { updateComposer({ valleyColor: v }); },
                swatches: ["#38bdf8", "#3b82f6", "#22d3ee", "#34d399", "#a78bfa"],
              })
            ),
            composerPreview
          ...
          createElement("div", { style: { display: "flex", flexDirection: "column", gap: "2px" } },
            createElement(ToggleRow, { label: t("composerEnabled"), checked: (cp && cp.enabled) !== false, onChange: function (v) { updateComposer({ enabled: v }); } }),
            createElement(SliderRow, {
              label: t("width") + " (px)",
              value: cp.width || 2,
              min: 1, max: 14,
              onChange: function (v) { updateComposer({ width: v }); },
            }),
            createElement(SliderRow, {
              label: t("glow"),
              value: cp.glow || 0,
              min: 0, max: 80,
              onChange: function (v) { updateComposer({ glow: v }); },
            }),
            createElement(SliderRow, {
              label: t("opacity"),
              value: Math.round((cp.opacity || 0.4) * 100),
              min: 20, max: 100,
              format: function (v) { return v + "%"; },
              onChange: function (v) { updateComposer({ opacity: v / 100 }); },
            }),
            createElement(SelectRow, {
              label: t("animation"),
              value: cp.animation || "breathing",
              onChange: function (v) { updateComposer({ animation: v }); },
              options: [
                { value: "breathing", label: t("animBreathing") },
                { value: "solid", label: t("animSolid") },
                { value: "off", label: t("animOff") },
            ...
            createElement(SelectRow, {
              ...
              value: cp.glowDirection || "out",
              onChange: function (v) { updateComposer({ glowDirection: v }); },
              options: [
                { value: "in", label: t("glowIn") },
                { value: "out", label: t("glowOut") },
              ],
            ...
            createElement(SliderRow, {
              label: t("breathingSpeed"),
              value: cp.breathingSpeed || 2.6,
              min: 0.5, max: 10, step: 0.1,
              format: function (v) { return v.toFixed(1) + "s"; },
              onChange: function (v) { updateComposer({ breathingSpeed: v }); },
            }),
            createElement(ToggleRow, { label: t("flow"), checked: cp.flow === true, onChange: function (v) { updateComposer({ flow: v }); } }),
            createElement(SliderRow, {
              label: t("flowSpeed"),
              value: cp.flowSpeed || 6,
              min: 2, max: 24, step: 0.5,
              format: function (v) { return v.toFixed(1) + "s"; },
              onChange: function (v) { updateComposer({ flowSpeed: v }); },
            })
          ),
          createElement("div", { style: { display: "flex", gap: "8px" } },
            createElement(Button, { size: "sm", variant: "ghost", onClick: function () { resetSection("composer"); } }, t("resetComposer"))
          )
        ),

        // ---- 状态角标（独立分区）
        createElement(Card, { title: t("badgeTitle"), desc: t("badgeDesc") },
          createElement(ToggleRow, { label: t("badgeEnabled"), checked: (bd && bd.enabled) !== false, onChange: function (v) { updateBadge({ enabled: v }); } }),
          createElement("div", { style: { display: "flex", gap: "24px", flexWrap: "wrap" } },
            createElement("div", { style: { flex: "1 1 260px", minWidth: 0, display: "flex", flexDirection: "column", gap: "2px" } },
              createElement("span", { style: { fontSize: 13, fontWeight: 600, color: "var(--dsw-alias-label-primary)", marginTop: "6px" } }, t("badgeDesktop")),
              createElement(SelectRow, {
                label: t("badgeCorner"),
                value: (bd.desktop && bd.desktop.corner) || "br",
                onChange: function (v) { updateBadge({ desktop: Object.assign({}, bd.desktop || {}, { corner: v }) }); },
                options: [
                  { value: "tl", label: t("cornerTL") },
                  { value: "tr", label: t("cornerTR") },
                  { value: "bl", label: t("cornerBL") },
                  { value: "br", label: t("cornerBR") },
                ],
              }),
              createElement(SliderRow, {
                label: t("badgeX"),
                value: (bd.desktop && Number(bd.desktop.x)) || 18,
                min: 0, max: 200,
                format: function (v) { return v + "px"; },
                onChange: function (v) { updateBadge({ desktop: Object.assign({}, bd.desktop || {}, { x: v }) }); },
              }),
              createElement(SliderRow, {
                label: t("badgeY"),
                value: (bd.desktop && Number(bd.desktop.y)) || 18,
                min: 0, max: 400,
                format: function (v) { return v + "px"; },
                onChange: function (v) { updateBadge({ desktop: Object.assign({}, bd.desktop || {}, { y: v }) }); },
              })
            ),
            createElement("div", { style: { flex: "1 1 260px", minWidth: 0, display: "flex", flexDirection: "column", gap: "2px" } },
              createElement("span", { style: { fontSize: 13, fontWeight: 600, color: "var(--dsw-alias-label-primary)", marginTop: "6px" } }, t("badgeMobile")),
              createElement(SelectRow, {
                label: t("badgeCorner"),
                value: (bd.mobile && bd.mobile.corner) || "br",
                onChange: function (v) { updateBadge({ mobile: Object.assign({}, bd.mobile || {}, { corner: v }) }); },
                options: [
                  { value: "tl", label: t("cornerTL") },
                  { value: "tr", label: t("cornerTR") },
                  { value: "bl", label: t("cornerBL") },
              ...
              createElement(SliderRow, {
                label: t("badgeX"),
                value: (bd.mobile && Number(bd.mobile.x)) || 12,
                min: 0, max: 200,
                format: function (v) { return v + "px"; },
                onChange: function (v) { updateBadge({ mobile: Object.assign({}, bd.mobile || {}, { x: v }) }); },
              }),
              createElement(SliderRow, {
                label: t("badgeY"),
                value: (bd.mobile && Number(bd.mobile.y)) || 96,
                min: 0, max: 400,
                format: function (v) { return v + "px"; },
                onChange: function (v) { updateBadge({ mobile: Object.assign({}, bd.mobile || {}, { y: v }) }); },
              })
            )
          ),
          createElement("div", { style: { display: "flex", gap: "8px" } },
            createElement(Button, { size: "sm", variant: "ghost", onClick: function () { resetSection("badge"); } }, t("resetBadge"))
          )
        ),
    ...
    exports._runtime = runtime; // test hook: drive polls in the runtime test
